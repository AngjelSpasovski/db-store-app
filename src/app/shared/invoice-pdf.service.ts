...
export class InvoicePdfService {
  ...
  generateInvoicePdf(inv: InvoiceDto, billingDetails?: InvoiceBillingDetails | null): void {
    ...
    // ===== Footer disclaimer ============================================
    // ===== Frame + Footer (meta + disclaimer) ============================
    // (ставено на крај за да работи и со autoTable multi-page)
    const totalPages =
      (doc as any).getNumberOfPages?.() ??
      (doc as any).internal?.getNumberOfPages?.() ??
      1;

    const frameInset = 18; // минимална рамка
    const frameX = frameInset;
    const frameY = frameInset;
    const frameW = pageW - frameInset * 2;
    const frameH = pageH - frameInset * 2;

    // footer area (внатре во рамката)
    const footerAreaH = 44;
    const footerTopY = frameY + frameH - footerAreaH;

    // padding внатре во footer-от
    const footerPadX = 12;
    ...
    for (let p = 1; p <= totalPages; p++) {
      ...
      // --- Minimal frame ---
      setDraw([235, 238, 244]);      // малце понежно од clrBorder
      ...
      doc.line(frameX + footerPadX, footerTopY, frameX + frameW - footerPadX, footerTopY);

      // --- Footer meta (left/center/right) ---
      doc.setFontSize(8);
      ...
      doc.text(rightMeta, frameX + frameW - footerPadX, metaY, { align: 'right' });

      // --- Disclaimer (max 2 lines, да не се лепи на рамката) ---
      doc.setFontSize(7.5);
      setTextColor([135, 142, 154]);

      const maxDiscW = frameW - footerPadX * 2;
      let discLines = doc.splitTextToSize(disc, maxDiscW) as string[];

      const MAX_LINES = 2;
      if (discLines.length > MAX_LINES) {
        discLines = discLines.slice(0, MAX_LINES);
        // мала “елипса” за да се знае дека има уште
        discLines[MAX_LINES - 1] = `${discLines[MAX_LINES - 1]}…`;
      }

      doc.text(discLines, frameX + footerPadX, discY);
    }
